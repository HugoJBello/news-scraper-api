...
import { GlobalConfigSqlSqlI } from '../models/GlobalConfigSql';

export async function get(req: Request, res: Response): Promise<void> {
  const params = req.query;
  if (params.id) {
    const id: string = params.id as string;
    const result = await globalConfigService.findOne({ id }).catch((err) => {
      Logger.error(err);
      res.status(400);
    });
    res.status(200).send({
...
//http://localhost:3000/api/v1/globalConfig/findQuery?newspaper=eldiario.es&limit=6&orderByParam=createdAt&orderDirection=DESC
export const findQuery = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const result = await globalConfigService.findQuery(
      query,
      order,
      offset,
      limit
...
//http://localhost:3000/api/v1/globalConfig/saveOrUpdate
export const saveOrUpdate = async (
  req: Request,
  res: Response
): Promise<void> => {
  const obj = req.body as GlobalConfigSqlSqlI;

  try {
    const result = await globalConfigService.saveOrUpdate(obj);
    res.status(200).send({
      success: true,
      payload: result
    });
